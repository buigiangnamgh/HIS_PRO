    ...
    public partial class Mps000007Processor : AbstractProcessor
    {
        ...
        void ProcessSingleKey()
        {
            try
            {
                ...
                filter.TREATMENT_ID = this.rdo.Treatment.ID;
                filter.ORDER_FIELD = "ID";
                filter.ORDER_DIRECTION = "ASC";
                CommonParam param = new CommonParam();
                HIS_DHST dhstResult = new Inventec.Common.Adapter.BackendAdapter(param).Get<List<HIS_DHST>>("api/HisDhst/Get", HIS.Desktop.ApiConsumer.ApiConsumers.MosConsumer, filter, param).ToList().FirstOrDefault();
                if (dhstResult != null)
                {
                    SetSingleKey((new KeyValue(Mps000007ExtendSingleKey.PULSE_2, dhstResult.PULSE)));
                    SetSingleKey((new KeyValue(Mps000007ExtendSingleKey.TEMPERATURE_2, dhstResult.TEMPERATURE)));
                    SetSingleKey((new KeyValue(Mps000007ExtendSingleKey.BLOOD_PRESSURE_MAX_2, dhstResult.BLOOD_PRESSURE_MAX)));
                    SetSingleKey((new KeyValue(Mps000007ExtendSingleKey.BLOOD_PRESSURE_MIN_2, dhstResult.BLOOD_PRESSURE_MIN)));
                    SetSingleKey((new KeyValue(Mps000007ExtendSingleKey.BREATH_RATE_2, dhstResult.BREATH_RATE)));
                    SetSingleKey((new KeyValue(Mps000007ExtendSingleKey.WEIGHT_2, dhstResult.WEIGHT)));
                    SetSingleKey((new KeyValue(Mps000007ExtendSingleKey.HEIGHT_2, dhstResult.HEIGHT)));
                    Inventec.Common.Logging.LogSystem.Debug("------dhstResult is NOT null");
                }
                else
                {
                    Inventec.Common.Logging.LogSystem.Debug("------dhstResult null");
                }
                AddObjectKeyIntoListkey<HIS_DHST>(rdo.DHST, false);
