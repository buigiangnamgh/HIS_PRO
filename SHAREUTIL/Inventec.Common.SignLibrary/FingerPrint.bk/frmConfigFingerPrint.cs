...
namespace Demo
{
    public partial class frmConfigFingerPrint : Form
    {
        ...
        Action<byte[]> actSendSignPadImage;

        [DllImport("user32.dll", EntryPoint = "SendMessageA")]
        public static extern int SendMessage(IntPtr hwnd, int wMsg, IntPtr wParam, IntPtr lParam);

        public frmConfigFingerPrint()
        {
            InitializeComponent();
        }
        public frmConfigFingerPrint(Action<byte[]> _actSendSignPadImage)
        {
            InitializeComponent();
            this.actSendSignPadImage = _actSendSignPadImage;
        }
        private void bnInit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int init = zkfp2.Init();
                if ((ret = init) == zkfperrdef.ZKFP_ERR_OK)
                {
        ...
        private void btnChoose_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.actSendSignPadImage != null)
                {
                    this.actSendSignPadImage(ms.ToArray());
                }
            }
            catch (Exception ex)
            {
