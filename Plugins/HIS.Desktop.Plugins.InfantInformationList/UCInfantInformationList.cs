...
namespace HIS.Desktop.Plugins.InfantInformationList
{
    public partial class UCInfantInformationList : UserControlBase
    {
        ...
        private void PrintGiayChungSinh(V_HIS_BABY baby)
        {
            if (baby != null)
            {
        ...
        private void ButtonEdit_InGiayChungSinh_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                V_HIS_BABY printBaby = (V_HIS_BABY)gridView1.GetFocusedRow();
                if (printBaby != null)
                {
                    PrintGiayChungSinh(printBaby);
                }
            }
            catch (Exception ex)
            {
        ...
        private bool InPhieuChungSinh(string printTypeCode, string fileName)
        {
            ...
            try
            {
                ...
                if (printBaby != null)
                {
                    ...
                    Inventec.Common.SignLibrary.ADO.InputADO inputADO = new HIS.Desktop.Plugins.Library.EmrGenerate.EmrGenerateProcessor().GenerateInputADOWithPrintTypeCode((printTreatment != null ? printTreatment.TREATMENT_CODE : ""), printTypeCode, currentModule.RoomId);
        ...
        private void gridView1_CustomRowCellEdit(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            try
            {
                ...
                if (e.RowHandle >= 0)
                {
                    V_HIS_BABY data = (V_HIS_BABY)gridView1.GetRow(e.RowHandle);
   
                    if (e.Column.FieldName == "PRINT")
                    {
                        e.RepositoryItem = ButtonEdit_InGiayChungSinh;
        ...
        private void gridView1_Click(object sender, EventArgs e)
        {
            try
            {
                var rowData = (MOS.EFMODEL.DataModels.V_HIS_BABY)gridView1.GetFocusedRow();
                ...
                if ((Control.ModifierKeys & Keys.Control) != Keys.Control)
                {
                    ...
                    if (hi.InRowCell)
                    {
                        if (hi.Column.RealColumnEdit.GetType() == typeof(DevExpress.XtraEditors.Repository.RepositoryItemButtonEdit))
                        {
                            ...
                            var dataRow = (V_HIS_BABY)gridView1.GetRow(rowHandle);
                            if (dataRow != null)
                            {
                                PrintGiayChungSinh(dataRow);
                            }
