    ...
    public partial class frmApprovalExamSpecialist : FormBase
    {
            ...
            try
            {
                ...
                Action myaction = () =>
                {
                    ...
                    if (_currentConfigApp != null)
                    {
                        ...
                        appUserFilter.LOGINNAME = Inventec.UC.Login.Base.ClientTokenManagerStore.ClientTokenManager.GetLoginName();
        ...
        private void Mps000062(string printTypeCode, string fileName, ref bool result, bool resultPrintAndSign = false)
        {
            try
            {
                #region ----
                WaitingManager.Show();
                V_HIS_TRACKING currentTrackingForPrint = GetVHisTrackingByID(this.currentSpecialistExam.TRACKING_ID ?? 0);
                Inventec.Common.Logging.LogSystem.Debug(Inventec.Common.Logging.LogUtil.TraceData("private void Mps000062 currentTrackingForPrint", currentTrackingForPrint));
                CommonParam param = new CommonParam();
        ...
        private void CreateThreadLoadData(object param)
        {
            ...
            Thread threadResumTracking = new System.Threading.Thread(new System.Threading.ParameterizedThreadStart(LoadDataResumTrackingNewThread));
        ...
        private void LoadDataResumTrackingNewThread(object param)
        {
            try
            {
                LoadDataResumTracking((long)param);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
        ...
        private void LoadDataResumTracking(long treatmentId)
        {
            CommonParam param = new CommonParam();
            HisTrackingFilter trackingFilter = new HisTrackingFilter();
            trackingFilter.ID = currentSpecialistExam.TRACKING_ID;
            this.currentTracking = new BackendAdapter(param).Get<List<HIS_TRACKING>>("api/HisTracking/Get", ApiConsumers.MosConsumer, trackingFilter, param).FirstOrDefault();
        }
