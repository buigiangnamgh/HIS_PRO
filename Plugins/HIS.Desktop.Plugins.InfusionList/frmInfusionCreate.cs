    ...
    public partial class frmInfusionCreate : HIS.Desktop.Utility.FormBase
    {
        ...
        private List<MEDITYPE> FillDataCombo()
        {
            try
            {
                ...
                List<HIS_SERVICE_REQ> ServiceReqList = new List<HIS_SERVICE_REQ>();
                if (lstExpMestMedicine6 != null && lstExpMestMedicine6.Count > 0)
                {
                    MOS.Filter.HisServiceReqFilter svFilter = new HisServiceReqFilter();
                    svFilter.IDs = lstExpMestMedicine6.Select(o => o.TDL_SERVICE_REQ_ID ?? 0).Distinct().ToList();
                    ServiceReqList = new BackendAdapter(new CommonParam()).Get<List<HIS_SERVICE_REQ>>("api/HisServiceReq/Get", ApiConsumer.ApiConsumers.MosConsumer, svFilter, null);
                }
                ...
                foreach (V_HIS_EXP_MEST_MEDICINE_6 exp in lstExpMestMedicine6)
                {
                    ...
                    medi.ngoaikho = false;
                    if (ServiceReqList != null && ServiceReqList.Count() > 0 && exp.TDL_SERVICE_REQ_ID.HasValue)
                    {
                        var sv = ServiceReqList.FirstOrDefault(o => o.ID == exp.TDL_SERVICE_REQ_ID);
                        if (sv != null)
                        {
                            medi.USE_TIME = sv.USE_TIME;
                        }
                    }
                    glstMedi.Add(medi);
                }
                ...
                {
                    List<HIS_SERVICE_REQ> ServiceReqOutList = new List<HIS_SERVICE_REQ>();
                    if (outMestMedicines != null && outMestMedicines.Count > 0)
                    {
                        MOS.Filter.HisServiceReqFilter svFilter = new HisServiceReqFilter();
                        svFilter.IDs = outMestMedicines.Select(o => o.TDL_SERVICE_REQ_ID ?? 0).Distinct().ToList();
                        ServiceReqOutList = new BackendAdapter(new CommonParam()).Get<List<HIS_SERVICE_REQ>>("api/HisServiceReq/Get", ApiConsumer.ApiConsumers.MosConsumer, svFilter, null);
                    }
                ...
                if (glstMetyReq != null && glstMetyReq.Count > 0)
                {
                    ...
                    {
                        MOS.Filter.HisServiceReqFilter svFilter = new HisServiceReqFilter();
                        svFilter.IDs = glstMetyReq.Select(o => o.SERVICE_REQ_ID).Distinct().ToList();
                        ServiceReqOutList = new BackendAdapter(new CommonParam()).Get<List<HIS_SERVICE_REQ>>("api/HisServiceReq/Get", ApiConsumer.ApiConsumers.MosConsumer, svFilter, null);
                    }
                ...
                foreach (MEDITYPE medi in glstMedi)
                {
                    ...
                    if (medi.USE_TIME.HasValue && medi.USE_TIME > 0)
                    {
                        medi.USE_TIME_STR = Inventec.Common.DateTime.Convert.TimeNumberToTimeString(Inventec.Common.TypeConvert.Parse.ToInt64(medi.USE_TIME.ToString()));
                    }
                }
