    ...
    public partial class UCExecuteRoom : UserControlBase
    {
        ...
        private void StartEvent(ref bool isStart, L_HIS_SERVICE_REQ serviceReqInput)
        {
            try
            {
                ...
                //canh bao ngoai chuyen khoa
                if (serviceReqInput.SERVICE_REQ_STT_ID == IMSys.DbConfig.HIS_RS.HIS_SERVICE_REQ_STT.ID__CXL && (HisConfigCFG.SpecialityWarningOption == "1" || HisConfigCFG.SpecialityWarningOption == "2"))
                {
                    //thong tin chuyen khoa
                    var specialitys = BackendDataWorker.Get<MOS.EFMODEL.DataModels.HIS_SPECIALITY>() ?? new List<MOS.EFMODEL.DataModels.HIS_SPECIALITY>();
                    var spUser = new List<MOS.EFMODEL.DataModels.HIS_SPECIALITY>();
                    var spService = new List<MOS.EFMODEL.DataModels.HIS_SPECIALITY>();
                    //thong tin bac si
                    ...
                    if (loginname != null && employees != null)
                    {
                        var employee = employees.FirstOrDefault(o => o.LOGINNAME == loginname);
                        Inventec.Common.Logging.LogSystem.Debug("employee:" + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => employee), employee));
                        if (employee != null && !string.IsNullOrWhiteSpace(employee.SPECIALITY_CODES))
                        {
                            spUser = specialitys.Where(o => string.Format(";{0};", employee.SPECIALITY_CODES).Contains(string.Format(";{0};", o.SPECIALITY_CODE))).ToList();
                        }
                    }
                    //thong tin dich vu
                    if (!string.IsNullOrWhiteSpace(serviceReqInput.TDL_SERVICE_IDS))
                    {
                        List<long> serviceIds = serviceReqInput.TDL_SERVICE_IDS.Split(';').Select(o => Int64.Parse(o)).ToList();
                        if (serviceIds != null && serviceIds.Count > 0)
                        {
                            CommonParam pr = new CommonParam();
                            HIS.Desktop.Plugins.ExecuteRoom.ADO.HisServiceSpecFilter filter = new HIS.Desktop.Plugins.ExecuteRoom.ADO.HisServiceSpecFilter();
                            filter.SERVICE_IDs = serviceIds;

                            var mosUserConsummer = new Inventec.Common.WebApiClient.ApiConsumer(HisConfigCFG.MOS_USER_URI, GlobalVariables.APPLICATION_CODE);
                            mosUserConsummer.SetTokenCode(HIS.Desktop.ApiConsumer.ApiConsumers.MosConsumer.GetTokenCode());
                            var listServiceSpec = new BackendAdapter(pr).Get<List<HIS.Desktop.Plugins.ExecuteRoom.ADO.HIS_SERVICE_SPEC>>(
                                                     "api/HisServiceSpec/Get",
                                            mosUserConsummer,
                                            filter,
                                            pr);
                            Inventec.Common.Logging.LogSystem.Debug("HisServiceSpec:" + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => listServiceSpec), listServiceSpec));
                            if (listServiceSpec != null && listServiceSpec.Count>0)
                            {
                                var specialityIds = listServiceSpec.Select(o => o.SPECIALITY_ID).Distinct().ToList();
                                spService = specialitys.Where(o => specialityIds.Contains(o.ID)).ToList();
                            }
                        }
                    }

                    if (spUser.Count > 0 && spService.Count > 0 && specialitys.Where(o => spUser.Select(p => p.ID).Contains(o.ID) && spService.Select(p => p.ID).Contains(o.ID)).Count() == 0)
                    {
                        if (HisConfigCFG.SpecialityWarningOption == "1")
                        {
                            MessageBox.Show(string.Format("Chuyên khoa của người xử lý [{0}] khác với chuyên khoa của dịch vụ [{1}]", string.Join(",", spUser.Select(o => o.SPECIALITY_NAME).ToList()), string.Join(",", spService.Select(o => o.SPECIALITY_NAME).ToList())), Inventec.Desktop.Common.LibraryMessage.MessageUtil.GetMessage(Inventec.Desktop.Common.LibraryMessage.Message.Enum.TieuDeCuaSoThongBaoLaThongBao));
                            return;
                        }
                        else if (HisConfigCFG.SpecialityWarningOption == "2")
                        {
                            if (MessageBox.Show(string.Format("Chuyên khoa của người xử lý [{0}] khác với chuyên khoa của dịch vụ [{1}]. Bạn có muốn tiếp tục xử lý?", string.Join(",", spUser.Select(o => o.SPECIALITY_NAME).ToList()), string.Join(",", spService.Select(o => o.SPECIALITY_NAME).ToList())), Inventec.Desktop.Common.LibraryMessage.MessageUtil.GetMessage(Inventec.Desktop.Common.LibraryMessage.Message.Enum.TieuDeCuaSoThongBaoLaThongBao), MessageBoxButtons.YesNo) == DialogResult.No)
                                return;
                        }
                    }
                }
