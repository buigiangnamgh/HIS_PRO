...
namespace HIS.Desktop.Plugins.AssignServiceEdit
{
    public partial class FormAssignServiceEdit : HIS.Desktop.Utility.FormBase
    {
        ...
        private bool CheckBedService(MOS.EFMODEL.DataModels.HIS_SERVICE_REQ serviceReq, CommonParam param)
        {
            bool valid = true;
            ...
            try
            {
                ...
                long dt = Inventec.Common.DateTime.Convert.SystemDateTimeToTimeNumber(dtInstructionTime.DateTime) ?? 0;

                foreach (var item in bedlogs)
                {
                    if (item.FINISH_TIME <= dt)
                    {
                        valid = false;
                        MessageBox.Show("Thời gian chỉ định lớn hơn thời gian kết thúc giường", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        break;
                    }
                }
            }
        ...
        private void SaveProcess(bool isPrintNow)
        {
            try
            {
                ...
                valid = valid && CheckBedService(this.HisServiceReq, param);
