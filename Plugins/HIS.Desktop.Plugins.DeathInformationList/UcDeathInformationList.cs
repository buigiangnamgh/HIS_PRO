    ...
    public partial class UcDeathInformationList : UserControlBase
    {
        ...
        private void LoadGridData(object param)
        {
            try
            {
                ...
                Inventec.Common.Logging.LogSystem.Debug("HIS.Desktop.Plugins.DeathInformationList filter " + Inventec.Common.Logging.LogUtil.TraceData("DeathInformationList ", filter));
        ...
        private void SetFilter(ref HisTreatmentViewFilter filter)
        {
            try
            {
                ...
                {
                    Patientcode = string.Format("{0:0000000000}", System.Convert.ToInt64(Patientcode));
                ...
                if (dtCreateTimeFrom.EditValue != null && dtCreateTimeFrom.DateTime != DateTime.MinValue)
                    filter.DEATH_TIME_FROM = Inventec.Common.TypeConvert.Parse.ToInt64(
                        System.Convert.ToDateTime(dtCreateTimeFrom.EditValue).ToString("yyyyMMdd") + "000000");
                if (dtCreateTimeTo.EditValue != null && dtCreateTimeTo.DateTime != DateTime.MinValue)
                    filter.DEATH_TIME_TO = Inventec.Common.TypeConvert.Parse.ToInt64(
                        System.Convert.ToDateTime(dtCreateTimeTo.EditValue).ToString("yyyyMMdd") + "235959");
                if (this.dtDeathIssuedDateFrom.EditValue != null && this.dtDeathIssuedDateFrom.DateTime != System.DateTime.MinValue)
                    filter.DEATH_ISSUED_DATE_FROM = new long?(Parse.ToInt64(System.Convert.ToDateTime(this.dtDeathIssuedDateFrom.EditValue).ToString("yyyyMMdd") + "000000"));
                if (this.dtDeathIssuedDateTo.EditValue != null && this.dtDeathIssuedDateTo.DateTime != System.DateTime.MinValue)
                    filter.DEATH_ISSUED_DATE_TO = new long?(Parse.ToInt64(System.Convert.ToDateTime(this.dtDeathIssuedDateTo.EditValue).ToString("yyyyMMdd") + "235959"));
        ...
        private void SetDefaultControl()
        {
            try
            {
                ...
                this.dtDeathIssuedDateFrom.EditValue = (object)Inventec.Common.DateTime.Convert.TimeNumberToSystemDateTime(Get.StartMonth().GetValueOrDefault());
                this.dtDeathIssuedDateTo.EditValue = (object)Inventec.Common.DateTime.Convert.TimeNumberToSystemDateTime(Get.EndDay().GetValueOrDefault());
                txtPatientName.Text = "";
                txtPatientCode.Text = "";
        ...
        private void PrintMps000268(V_HIS_TREATMENT treatmentV)
        {
            try
            {
                ...
                if (treatmentV != null)
                {
                    ...
                    Inventec.Common.Mapper.DataObjectMapper.Map<HIS_TREATMENT>(treatment, treatmentV);
        ...
        private void ButtonEdit_InGiayBaoTuVong_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                V_HIS_TREATMENT treatment = (V_HIS_TREATMENT)gridView1.GetFocusedRow();
                if (treatment != null)
                {
                    PrintMps000268(treatment);
                }
            }
            catch (Exception ex)
            {
                ...
                if ((Control.ModifierKeys & Keys.Control) != Keys.Control)
                {
                    ...
                    if (hi.InRowCell)
                    {
                        if (hi.Column.RealColumnEdit.GetType() == typeof(DevExpress.XtraEditors.Repository.RepositoryItemButtonEdit))
                        {
                            ...
                            var dataRow = (V_HIS_TREATMENT)gridView1.GetRow(rowHandle);
                            if (dataRow != null)
                            {
                                PrintMps000268(dataRow);
                            }
