...
namespace HIS.Desktop.Plugins.CallPatientExpMest
{
    public partial class frmChooseRoomForWaitingScreen : HIS.Desktop.Utility.FormBase
    {
        ...
        internal MOS.EFMODEL.DataModels.V_HIS_EXP_MEST_2 HisServiceReq = null;
        ...
        private void tgExtendMonitor_Toggled(object sender, EventArgs e)
        {
            try
            {
                ...
                List<MOS.EFMODEL.DataModels.HIS_EXP_MEST_STT> serviceReqStts = new List<MOS.EFMODEL.DataModels.HIS_EXP_MEST_STT>();
                foreach (var item in serviceReqSttSdos)
                {
                    if (item.checkStt)
                    {
                        MOS.EFMODEL.DataModels.HIS_EXP_MEST_STT serviceReqStt = new MOS.EFMODEL.DataModels.HIS_EXP_MEST_STT();
                        AutoMapper.Mapper.CreateMap<ServiceReqSttSDO, MOS.EFMODEL.DataModels.HIS_EXP_MEST_STT>();
                        serviceReqStt = AutoMapper.Mapper.Map<ServiceReqSttSDO, MOS.EFMODEL.DataModels.HIS_EXP_MEST_STT>(item);
                ...
                {
                    HIS.Desktop.Plugins.CallPatientExpMest.ChooseRoomForWaitingScreenProcess.ShowFormInExtendMonitor(aFrmWaitingScreenQy);
                    this.Close();
                }
                else
                {
                    HIS.Desktop.Plugins.CallPatientExpMest.ChooseRoomForWaitingScreenProcess.TurnOffExtendMonitor(aFrmWaitingScreenQy);
                }
            }
            catch (Exception ex)
            {
