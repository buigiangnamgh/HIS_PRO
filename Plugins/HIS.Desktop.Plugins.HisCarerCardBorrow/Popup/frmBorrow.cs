    ...
    public partial class frmBorrow : Form
    {
        ...
        private void PrintTemp()
        {
            try
            {
                ...
                richEditorMain.RunPrintTemplate("Mps190000", DelegateRunPrinter);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
        ...
        private bool DelegateRunPrinter(string printTypeCode, string fileName)
        {
            ...
            try
            {
                ...
                {
                    case "Mps190000":
                        InTheNguoiNha(printTypeCode, fileName, ref result);
                        break;
                }
            }
            catch (Exception ex)
        ...
        private void InTheNguoiNha(string printTypeCode, string fileName, ref bool result)
        {
            try
            {
                ...
                CommonParam paramCommon = new CommonParam();
                var treatments = new Inventec.Common.Adapter.BackendAdapter(paramCommon).Get<List<V_HIS_TREATMENT_4>>("api/HisTreatment/GetView4", ApiConsumers.MosConsumer, filter, paramCommon);
                Mps.Processor.Mps190000.PDO.BorrowADO borrow = new Mps.Processor.Mps190000.PDO.BorrowADO();
                borrow.treatment = treatments.FirstOrDefault();
                short? status_ispause = borrow.treatment.IS_PAUSE;
                short? status_islock = borrow.treatment.IS_ACTIVE;
                short? status_islockhein = borrow.treatment.IS_LOCK_HEIN;
                //Status
                //1- dang dieu tri
                //2- da ket thuc
                //3- khóa hồ sơ
                ...
                if (status_islockhein != IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__TRUE)
                {
                    if (status_islock == IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__TRUE)
                    {
                        ...
                        {
                            borrow.TREATMENT_STT_NAME = "Đang điều trị";
                        }
                        else
                        {
                            borrow.TREATMENT_STT_NAME = "Kết thúc đt";
                        }
                    }
                    else
                    {
                        borrow.TREATMENT_STT_NAME = "Khóa hồ sơ";
                    }
                }
                else
                {
                    borrow.TREATMENT_STT_NAME = "Duyệt BHYT";
                }
                if (lstBorrowADO != null && lstBorrowADO.Count > 0)
                {
                    borrow.CARER_CARD_NUMBER = lstBorrowADO.FirstOrDefault().CARER_CARD_NUMBER;
                    borrow.TIME_FROM = lstBorrowADO.FirstOrDefault().TIME_FROM;
                }

                MPS.Processor.Mps190000.PDO.Mps190000PDO mps0000085RDO = new MPS.Processor.Mps190000.PDO.Mps190000PDO(
                borrow
                  );
                WaitingManager.Hide();
                ...
                PrintData = new MPS.ProcessorBase.Core.PrintData(printTypeCode, fileName, mps0000085RDO, MPS.ProcessorBase.PrintConfig.PreviewType.PrintNow, printerName);
