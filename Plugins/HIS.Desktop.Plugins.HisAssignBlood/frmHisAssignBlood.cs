    ...
    public partial class frmHisAssignBlood : HIS.Desktop.Utility.FormBase
    {
        ...
        private void SetEnableButtonControlBlood()
        {
            try
            {
                ...
                this.btnIntem.Enabled = (this.actionType == GlobalVariables.ActionView || this.actionType == GlobalVariables.ActionViewForEdit);
        ...
        private void btnIntem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (HisPrescriptionSDOPrint != null && HisPrescriptionSDOPrint.Count() > 0)
                {
                    InTemBarcodeMau(HisPrescriptionSDOPrint);
                }
                WaitingManager.Hide();
            }
            catch (Exception ex)
        ...
        private void InTemBarcodeMau(List<PatientBloodPresResultSDO> datas)
        {
            try
            {
                string txt = "";
        ...
        private void GenTextMau(PatientBloodPresResultSDO serviceReq, ref string txt)
        {
            MOS.Filter.HisExpMestFilter filter = new HisExpMestFilter();
            filter.SERVICE_REQ_ID = serviceReq.ServiceReq.ID;
            ...
            var department = BackendDataWorker.Get<HIS_DEPARTMENT>().FirstOrDefault(o => o.ID == serviceReq.ServiceReq.REQUEST_DEPARTMENT_ID);
            var room = BackendDataWorker.Get<V_HIS_ROOM>().FirstOrDefault(o => o.ID == serviceReq.ServiceReq.REQUEST_ROOM_ID);
            txt += "," + serviceReq.ServiceReq.TDL_PATIENT_NAME;
            txt += "," + serviceReq.ServiceReq.TDL_PATIENT_GENDER_NAME;
            txt += "," + serviceReq.ServiceReq.TDL_PATIENT_DOB;
            txt += "," + (serviceReq.ServiceReq.TDL_PATIENT_DOB > 10000000000000 ? serviceReq.ServiceReq.TDL_PATIENT_DOB.ToString().Substring(0, 4) : "");
            if (department != null)
            {
            ...
            txt += "," + serviceReq.ServiceReq.TDL_TREATMENT_CODE;

            // xuong dong (1 row trong db)
            txt += "\n";
        }

        private void bbtnPrintTem_ItemClick(object sender, ItemClickEventArgs e)
        {
