...
namespace HIS.Desktop.Plugins.RegisterExamKiosk.Popup.RegisterExemKiosk
{
    public partial class frmRegisterExamKiosk : HIS.Desktop.Utility.FormBase
    {
        ...
        public frmRegisterExamKiosk(Inventec.Desktop.Common.Modules.Module module, SettingADO stADO)
            : base(module)
        {
            InitializeComponent();
        ...
        public frmRegisterExamKiosk(InformationObjectADO data, HIS.Desktop.Common.DelegateRefreshData _setNull, Inventec.Desktop.Common.Modules.Module module, DelegateCloseForm_Uc closingForm, long patientTypeId, SettingADO stADO)
            : base(module)
        {
            InitializeComponent();
            try
        ...
        private void frmRegisterExamKiosk_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                toggleSwitch_ThanhToan.EditValue = true;
                timer1.Start();
                timer1.Interval = this.stADO.Time_Refresh * 1000;
                LoadPatientype();
                WaitingManager.Show();
        ...
        private void getService(V_HIS_EXECUTE_ROOM_1 data, HisCardSDO hisCard, ref List<ServiceADO> listService, bool IsRegisterClick = false)
        {
            try
            {
                //HIS_SERVICE_ROOM
                ...
                var serviceIdByPatysNew = listServicePatys
                ...
                List<ServiceADO> listServiceClone = new List<ServiceADO>();
                foreach (var item in services)
                {
                    decimal price = 0;
                    V_HIS_SERVICE_PATY sp = null;
                    if (item.BILL_PATIENT_TYPE_ID.HasValue && item.BILL_PATIENT_TYPE_ID.Value > 0)
                        sp = serviceIdByPatysNew.FirstOrDefault(o => o.SERVICE_ID == item.ID && o.PATIENT_TYPE_ID == item.BILL_PATIENT_TYPE_ID.Value); // nếu có đối tượng phụ thu mặc định thì lấy theo đt phụ thu
                    else
                        sp = serviceIdByPatysNew.FirstOrDefault(o => o.SERVICE_ID == item.ID);

                    if (sp != null)
                    {
                        price = sp.PRICE;
                    }
                    listServiceClone.Add(new ServiceADO(item, price)); // review
                }
                ...
                if (patientTypeId != 0)
                {
                    ...
                    {
                        listService = listServiceClone.Where(o => serviceIdByPatys != null && serviceIdByPatys.Contains(o.ID)).ToList();
                        ...
                        if (HisConfigCFG.PrimaryPatientType == 2 && IsRegisterClick && PrimaryTypeId > 0)
                        {
                           ...
                           .Where(o => o.PATIENT_TYPE_ID == this.PrimaryTypeId
                            ...
                            if (serviceIdByPatysPrimaryPatientType != null && serviceIdByPatysPrimaryPatientType.Count > 0)
                            {
                                ...
                                listService = new List<ServiceADO>();
                                foreach (var item in dicGroup)
                                {
                                    ...
                                    {
                                        listService.Add(listServiceClone.FirstOrDefault(o => o.ID == item.Key));
                                    }
                                }
                            }
                        }
        ...
        private void getServices()
        {
            try
            {
                ...
                this.services = BackendDataWorker.Get<MOS.EFMODEL.DataModels.V_HIS_SERVICE>().Where(o => o.SERVICE_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_SERVICE_TYPE.ID__KH && o.IS_ACTIVE == IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__TRUE).ToList();
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        private void getServiceReq(long treatmentId)
        {
            try
            {
                if (treatmentId > 0)
                {
                    var patientTypeCode = HisConfigs.Get<string>("HIS.HIS_PATIENT_TYPE.PATIENT_TYPE_CODE.BHYT");
                    var patientType_BYHT = this.lstPatientType.Where(o => o.IS_ACTIVE == IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__TRUE && o.PATIENT_TYPE_CODE == patientTypeCode).FirstOrDefault();
                    CommonParam param = new CommonParam();
                    ...
                    {
                        this.ServiceReqPrint = listServiceReq.Where(o => o.SERVICE_REQ_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_SERVICE_REQ_TYPE.ID__KH && o.TDL_PATIENT_TYPE_ID != patientType_BYHT.ID).OrderBy(o => o.INTRUCTION_TIME).ThenBy(o => o.ID).FirstOrDefault();
                    }
                }
            }
            catch (Exception ex)
        ...
        private void getSereServs(V_HIS_SERVICE_REQ data)
        {
            try
            {
                if (data != null)
                {
                    ...
                    //var today = DateTime.Now.Year.ToString() + month + day; Ngày bựa ni BN đăng ký qua tổng đài --> tổng đài tiếp đón gồm có thông tin cccd ---> ngày mai BN đến kiot ---> nhập cccd ---> In phiếu khám
        ...
        private void FillDataToQR()
        {
            try
            {
                Inventec.Common.Logging.LogSystem.Info("FillDataToQR");
                ...
                labelControl_ServiceReqCode.Text = "";
                pE_QR.Image = null;
                lblSoTien.Text = "";
                layoutControlItem_MaYLenh.Visibility = DevExpress.XtraLayout.Utils.LayoutVisibility.Never;
                ...
                if (ServiceReqPrint != null && ServiceReqPrint.ID > 0)
                {
                    MOS.Filter.HisSereServBillFilter ssBillFilter = new HisSereServBillFilter();
                    ssBillFilter.TDL_SERVICE_REQ_ID = ServiceReqPrint.ID;
                    ...
                    else if (ServiceReqPrint.TRANS_REQ_ID > 0)
                    {
                        ...
                        if (transReq != null && transReq.Count > 0)
                        {
                            ...
                            layoutControlItem_MaYLenh.Visibility = DevExpress.XtraLayout.Utils.LayoutVisibility.Always;
                            labelControl_ServiceReqCode.Text = "MÃ Y LỆNH: " + ServiceReqPrint.SERVICE_REQ_CODE;
                    ...
                    else if (isPaid && ServiceReqPrint.SERVICE_REQ_STT_ID != IMSys.DbConfig.HIS_RS.HIS_SERVICE_REQ_STT.ID__HT)
                    {
                        layoutControlItem_MaYLenh.Visibility = DevExpress.XtraLayout.Utils.LayoutVisibility.Always;
                        labelControl_ServiceReqCode.Text = "MÃ Y LỆNH: " + ServiceReqPrint.SERVICE_REQ_CODE;
                    }
        ...
        //private void GenerateControlByRegisterGate(List<V_HIS_EXECUTE_ROOM_1> data)
        //{
        //    try
        //    {
        //        WaitingManager.Show();
        ...
        //            }
        //            tileControl1.ColumnCount = 5;//(int)ConfigSettings.Columns;
        //            tileControl1.ItemSize = 250;//(int)ConfigSettings.SizeItem;
        ...
        private void LoadDataTileExamRoom(List<V_HIS_EXECUTE_ROOM_1> data)
        {
            try
            {
                Inventec.Common.Logging.LogSystem.Info("this.stADO.Pk_SizeTitle " + this.stADO.Pk_SizeTitle);
                if (data != null && data.Count > 0)
                {
                    data = data.Where(o => o.IS_USE_KIOSK == IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__TRUE).ToList();
                    var groupData = data.GroupBy(o => o.ROOM_GROUP_ID).ToList();
                    foreach (var grItem in groupData)
                    {
                        ...
                        {
                            var listSerrvice = new List<ServiceADO>();
                        ...
                        tileControl1.ColumnCount = (int)this.stADO.Pk_Columns;
                        tileControl1.ItemSize = (int)this.stADO.Pk_SizeItem;
                        tileControl1.Groups.Add(group);
                    }
                }
            }
        ...
        void ItemClick(object sender, TileItemEventArgs e)
        {
            try
            {
                ...
                if (Inventec.Common.String.CheckString.IsOverMaxLengthUTF8(txtPhone.Text, 12))
                {
                    MessageBox.Show("Số điện thoại lớn hơn 12 kí tự", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    txtPhone.Focus();
                    txtPhone.SelectAll();
                    return;
                }
                ...
                this.vlistService = new List<ServiceADO>();
                ...
                if (hisCardPatientSdo != null)
                {
                    hisCardPatientSdo.Phone = txtPhone.Text.Trim();
                }
        ...
        private void btnRegister_Click(object sender, EventArgs e, V_HIS_SERVICE vhisService, V_HIS_EXECUTE_ROOM_1 executeRoom)
        {
            try
            {
                ...
                sdo.CardSDO.Phone = txtPhone.Text.Trim();
        ...
        //        var hisPatient = PrintGlobalStore.GetPatientById(treatmentPrint.PATIENT_ID);
        ...
        private void LoadBieuMauPhieuYeuCauKham(string printTypeCode, string fileName, ref bool result)
        {
            try
            {
                ...
                {
                    ratio = GetDefaultHeinRatio(patyAlter.HEIN_CARD_NUMBER, patyAlter.TREATMENT_TYPE_CODE, patyAlter.LEVEL_CODE, patyAlter.RIGHT_ROUTE_CODE);
                }

                var room = BackendDataWorker.Get<V_HIS_ROOM>().FirstOrDefault(o =>
                   o.ID == treatmentPrint.TDL_FIRST_EXAM_ROOM_ID);

                var tranInReason = BackendDataWorker.Get<HIS_TRAN_PATI_REASON>().FirstOrDefault(o => o.ID == treatmentPrint.TRANSFER_IN_REASON_ID);

                var inroom = BackendDataWorker.Get<V_HIS_ROOM>().FirstOrDefault(o => o.ID == treatmentPrint.IN_ROOM_ID);

                MPS.Processor.Mps000001.PDO.Mps000001ADO ado = new MPS.Processor.Mps000001.PDO.Mps000001ADO();
                ado.firstExamRoomName = (room != null ? room.ROOM_NAME : "");
                ado.ratio_text = ratio * 100 + " %";
                ...
                //{
                //    var roomSdo = ReqMaxNumOrderSDO.FirstOrDefault(o => o.EXECUTE_ROOM_ID == ServiceReqPrint.EXECUTE_ROOM_ID);
                //    if (roomSdo != null)
                //    {
                ...
                //if (dicSereServData != null && dicSereServData.ContainsKey(ServiceReqPrint.ID))
                //{
                ...
                if (sereServs != null && sereServs.Count > 0)
                {
                    ...
                    if (dataSS != null && dataSS.Count > 0)
                    {
                        ...
                        if (_service != null)
                        {
                            var serviceN = GetService(_service.PARENT_ID.Value);
                            ado.PARENT_NAME = serviceN != null ? serviceN.SERVICE_NAME : "";
                        }
                    }
                }

                V_HIS_ROOM room1 = BackendDataWorker.Get<V_HIS_ROOM>().FirstOrDefault(o => o.ID == ServiceReqPrint.EXECUTE_ROOM_ID);
                if (room1 != null)
                {
                    ado.ExecuteRoom = room1;
                }
                else
                {
                ...
                if (ServiceReqPrint != null && sereServs != null && sereServs.Count > 0)
                {
                    if (treatmentPrint != null && treatmentPrint.HAS_CARD == 1)
                    {
                        ...
                        if (hisCardList != null && hisCardList.Count > 0)
                        {
                            PrintKiosk printKiosk = new PrintKiosk(this.currentModule, patyAlter, ServiceReqPrint, sereServs, (DelegateReturnSuccess)DelegateSuccess, printTypeCode, fileName, treatmentPrint, null, null, null, this.checkPrint, hisCardList);
                            printKiosk.PrintProcess();
                        }
                    }
                    else
                    {
                        PrintKiosk printKiosk = new PrintKiosk(this.currentModule, patyAlter, ServiceReqPrint, sereServs, (DelegateReturnSuccess)DelegateSuccess, printTypeCode, fileName, treatmentPrint, null, null, null, this.checkPrint);
                        //printKiosk.PrintMps25();
                        printKiosk.PrintProcess();
        ...
        private void toggleSwitch_ThanhToan_Toggled(object sender, EventArgs e)
        {
            if ((bool)(toggleSwitch_ThanhToan.EditValue) == true)
            {
                timer1.Start();
        ...
        private void CreateThreadThanhToanRefresh()
        {
            Thread thread = new System.Threading.Thread(new System.Threading.ThreadStart(ThanhToanNewThreadRefresh));
            try
            {
                thread.Start();
            }
        ...
        private void ThanhToanNewThreadRefresh()
        {
            try
            {
                FillDataToQR();
