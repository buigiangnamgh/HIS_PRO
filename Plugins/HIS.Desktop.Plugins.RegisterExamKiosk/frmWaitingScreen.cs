...
namespace HIS.Desktop.Plugins.RegisterExamKiosk
{
    public partial class frmWaitingScreen : HIS.Desktop.Utility.FormBase
    {
        ...
        public void frmWaitingScreen_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                try
                {
                    ...
                    {
                        //this.layoutControlGroup2.BackgroundImage = listImage[0];
        ...
        #region Method
        private void getImageFromFile1()
        {
            try
            {
                ...
                if (System.IO.Directory.Exists(Inventec.UC.ImageLib.Base.LocalStore.LOCAL_STORAGE_PATH_KIOSK))
                {
                    ...
                    Inventec.Common.Logging.LogSystem.Info("HIS.Desktop.Plugins.RegisterExamKiosk getImageFromFile " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => fileEntries), fileEntries));
                    ...
                    else
                    {
                        foreach (var item in fileEntries)
                        {
                            using (FileStream fs = new FileStream(item, FileMode.Open, FileAccess.Read))
                            {
                                ...
                                {
                                    Inventec.Common.Logging.LogSystem.Info("HIS.Desktop.Plugins.RegisterExamKiosk getImageFromFile original " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => original), original));
                                    if (original != null)
                                    {
                                        listImage.Add(original);
                                    }
                                    original.Dispose();
                                }
                                fs.Close();
                            ...
                            //listImage.Add(Image.FromFile(item, true));
                        }
                        Inventec.Common.Logging.LogSystem.Info("HIS.Desktop.Plugins.RegisterExamKiosk getImageFromFile listImage " + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => listImage), listImage));
                    }
                }
                else
                {
