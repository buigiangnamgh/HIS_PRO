...
namespace HIS.UC.UCPatientRaw.Valid
{
    ...
    class Valid_CCCD_Control : DevExpress.XtraEditors.DXErrorProvider.ValidationRule
    {
        internal DevExpress.XtraEditors.TextEdit txtCCCD;
        internal bool isRequied = false;
        public override bool Validate(Control control, object value)
        {
            bool valid = true;
            Inventec.Common.Logging.LogSystem.Info("isRequied " + isRequied);
            Inventec.Common.Logging.LogSystem.Info("txtCCCD.Text " + txtCCCD.Text);
            try
            {
                valid = valid && (txtCCCD != null);
                if (valid)
                {
                    string strError = "";
                    string Cccd = txtCCCD.Text.Trim();
                    int len = Encoding.UTF8.GetByteCount(Cccd);
                    Inventec.Common.Logging.LogSystem.Info("Cccd.Length " + len);
                    if (isRequied)
                    {
                        ...
                        {
                            valid = false;
                            strError = "Độ dài số CCCD phải là 12 số";
                        }
                    }
                    else
                    {
                        if (!String.IsNullOrEmpty(Cccd) && len != 12)
                        {
                            valid = false;
                            strError = "Độ dài số CCCD phải là 12 số";
                        }
