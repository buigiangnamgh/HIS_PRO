...
{
	internal class Template7 : IRunTemplate
	{
		private ElectronicBillDataInput DataInput;

		public Template7(ElectronicBillDataInput dataInput)
		{
			DataInput = dataInput;
		}
		...
		public object Run()
		{
			...
			try
			{
				if (DataInput.SereServBill != null && DataInput.SereServBill.Count > 0)
				{
					...
					{
						List<HIS_SERE_SERV_BILL> list3 = DataInput.SereServBill.Where((HIS_SERE_SERV_BILL o) => o.TDL_PATIENT_TYPE_ID == HisPatientTypeCFG.PATIENT_TYPE_ID__BHYT && o.PRICE - o.TDL_TOTAL_PATIENT_PRICE_BHYT.GetValueOrDefault() >= 0m).ToList();
						List<HIS_SERE_SERV_BILL> list4 = DataInput.SereServBill.Where((HIS_SERE_SERV_BILL o) => o.TDL_PATIENT_TYPE_ID != HisPatientTypeCFG.PATIENT_TYPE_ID__BHYT || o.PRICE - o.TDL_TOTAL_PATIENT_PRICE_BHYT.GetValueOrDefault() < 0m).ToList();
						if (list3 != null && list3.Count > 0)
						{
							...
							productBase.Amount = Inventec.Common.Number.Convert.NumberToNumberRoundMax4(list3.Sum((HIS_SERE_SERV_BILL o) => o.TDL_TOTAL_PATIENT_PRICE_BHYT.GetValueOrDefault()));
							...
							List<HIS_SERE_SERV_BILL> list5 = list3.Where((HIS_SERE_SERV_BILL o) => o.PRICE - o.TDL_TOTAL_PATIENT_PRICE_BHYT.GetValueOrDefault() > 0m).ToList();
							if (list5 != null && list5.Count > 0)
							{
								var list6 = (from o in list5
								...
								foreach (var item in list6)
								{
									...
									productBase2.ProdName = ((val != null) ? (val.HEIN_SERVICE_BHYT_NAME ?? val.SERVICE_NAME) : item.First().TDL_SERVICE_NAME);
									productBase2.ProdQuantity = item.Sum((HIS_SERE_SERV_BILL s) => s.TDL_AMOUNT.GetValueOrDefault());
									productBase2.Amount = Inventec.Common.Number.Convert.NumberToNumberRoundMax4(item.Sum((HIS_SERE_SERV_BILL s) => s.PRICE - s.TDL_TOTAL_PATIENT_PRICE_BHYT.GetValueOrDefault()));
						...
						{
							var list7 = (from o in list4
								group o by new { o.TDL_SERVICE_ID, o.TDL_REAL_PRICE }).ToList();
							foreach (var item2 in list7)
							{
					...
					{
						var list8 = (from o in DataInput.SereServBill
							group o by new { o.TDL_SERVICE_ID, o.TDL_REAL_PRICE }).ToList();
						foreach (var item3 in list8)
						{
